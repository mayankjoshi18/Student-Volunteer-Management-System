...
export default function Signup() {
  ...
  const onSubmit = async (data: any) => {
    try {
      const user = await signup(data.name, data.email, data.password, role, data.department, role === 'student' ? data.studentId : undefined);
      navigate(`/${user.role}`);
    } catch (err) {
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12">
        <div className="max-w-md w-full space-y-6 bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800/80 p-8 rounded-3xl shadow-sm">
          ...
          <form id="signup-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* Password */}
            <div className="space-y-1">
              <label htmlFor="signup-password" className="text-xs font-semibold text-gray-500 dark:text-gray-400">Password</label>
              <input
                id="signup-password"
                type="password"
                {...register('password', {
